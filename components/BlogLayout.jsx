...
export default function BlogLayout({ children, title, category, date, image, imageAlt, tags, slug }) {
  ...
  return <><Navbar /><main id="main-content" className="interior-page article-page">
    <header className="article-header"><div className="studio-wrap"><Link className="page-breadcrumb" href="/blog/">← Resource library</Link><p className="studio-eyebrow">{category}</p><h1>{title}</h1><div className="article-byline"><img src="/images/kashif-portrait-ivory-v1.webp" alt="" width="40" height="40" /><span>Muhammad Kashif <small>{date}</small></span></div>
      {image && <img className="article-cover" src={image} alt={imageAlt || title} width="1600" height="900" fetchPriority="high" />}
    </div></header>
    <div className="studio-wrap article-grid"><div><article ref={articleRef} className="prose-custom">{children}</article><CommentSection postSlug={slug} /></div><aside className="article-sidebar">
      ...
      <div className="sidebar-card"><h2>Make it work for your product.</h2><p>Get design support for your listing, A+ Content or brand store.</p><Link className="studio-button" href="/services/">Explore services ↗</Link></div>
